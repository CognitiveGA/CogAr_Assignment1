...
- **Structural Risk Assessment** (Component: ``Rescue Structural Analysis``):
  ...
  - Required: ``Linear acceleration``and ``Angular Velocity``from ``IMUs``.
  - Provided: Information to Autonomous Navigation, Structural Condition to Real-time Reporting
...
- ``Structural Analysis`` → ``Core System``: 
- ``Structural Analysis`` → ``Communicator``: 
